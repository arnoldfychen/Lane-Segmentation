...
def UnetBase(tf.keras.Model):
    def __init__(self):
        super(UnetBase,self)._init__()
    ...
    def call(self,img,label_number,img_size):    
        ...
        start_pool = tf.keras.backend.pool2d(x=start_conv, pool_size=3, pool_stride=(2,2), padding='same', pool_mode='max')
        ...
        Output Coder
        """
        decode_conv5 = tf.compat.v1.image.resize_bilinear(images=decode_conv4, size=img_size)
